...
try:
    ...
    from NiftyPy.NiftyRec import (
        TR_grid_from_box_and_affine as __TR_grid_from_box_and_aff,
    )
    ...
    print(
        "NiftyPy could not be loaded: it will not be possible to reconstruct PET data. "
    )
    has_NiftyPy = False
...
def resample_image_on_grid(
    ...
    resampled_data = __TR_resample_grid(
        __np.float32(image.data),
        __np.float32(grid.data),
        __np.float32(affine.data),
        background,
        use_gpu,
        interpolation_mode,
    )
