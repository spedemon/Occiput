...
class Transform_Affine(object):
    ...
    def __get_inverse(self):
        inverse = Transform_Affine(
            data=self.__inverse, map_to=self.map_from, map_from=self.map_to
        )
        return inverse
...
class GridND(object):
    def __init__(
        self, data=None, space="", is_uniform=True, is_affine=True, is_axis_aligned=True
    ):
        self.ndim = None
    ...
    def corners(self, homogeneous_coords=False):
        if self.__corners is None:
            ...
            for i in range(n_corners):
                b = eval(
                    "["
                    + bin(i)[2:].zfill(self.ndim).replace("0", "0,").replace("1", "1,")
                    + "]"
                )
                b = (numpy.asarray(self.data.shape[0 : self.ndim]) - 1) * numpy.asarray(
                    b
                )
                s = str(b.tolist())[1:-1]
...
class Image3D(ImageND):
    ...
    def export_image(
        self, index, axis=0, normalise=True, scale_factor=None, shrink=None, rotate=0
    ):
        pass
